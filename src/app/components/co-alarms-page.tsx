...
import CoAlarmsNewPage from "./co-alarms-new-page";
import CoAlarmsNewPageMobile from "./co-alarms-new-page-mobile";
...
export default function CoAlarmsPage() {
  ...
  const [useNewPage, setUseNewPage] = useState(() => {
    try {
      return localStorage.getItem("coAlarmsPageVersion") !== "old";
    } catch {
      return true;
    }
  });
  ...
  const togglePageVersion = useCallback(() => {
    setUseNewPage((prev) => {
      const next = !prev;
      try {
        localStorage.setItem("coAlarmsPageVersion", next ? "new" : "old");
      } catch {}
      return next;
    });
  }, []);
  ...
  return (
    <div className="min-h-screen w-full">
      {useNewPage ? (
        isMobile ? <CoAlarmsNewPageMobile /> : <CoAlarmsNewPage />
      ) : isMobile ? (
